...
def create_graph(graph_type, N, graph_params=None, seed=None):
    """
    pravi graf zadatog tipa i velicine
    graph_type: e-r, ring, watts_str, barabasi
    N: broj cvorova
    graph_params: dodatni parametri grafa (npr. p, k, m...)
    seed: random
    """
    if graph_params is None:
        graph_params = {}
    if graph_type == 'erdos_renyi':
        # p je verovatnoca da postoji veza izmedju dva cvora
        p = graph_params.get('p', 0.1)
        return nx.erdos_renyi_graph(N, p, seed=seed)
    elif graph_type == 'ring':
        # pravi prstenastu strukturu (svaki cvor povezan sa sledecim)
...
def spectral_gap(G):
    """
    racuna spektralni zazor laplasijana grafa (lampda_2)
    ako graf nije povezan, lampda_2 ce biti 0
    """

     # pravi laplasijan matrice grafa (L = D - A)
    L = nx.laplacian_matrix(G).toarray()
     # racuna sve sopstvene vrednosti (mogu biti kompleksne)
    eigs = np.linalg.eigvals(L)
    # sortira ih po realnom delu
